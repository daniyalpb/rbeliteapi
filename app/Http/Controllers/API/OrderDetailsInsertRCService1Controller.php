<?php

namespace App\Http\Controllers\API;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use DB;
use Responce;
use validator;
use Redirect;
use Session;
use URL;
use Mail;

class OrderDetailsInsertRCService1Controller extends NewInitialController
{
    	public function insertorderdetailsRCService1(Request $req){	
		  try{
        	$query = DB::select('call usp_order_details_insert_RC_Service_1(?,?,?,?,?,?,?,?,?,?,?)',
                array($req->prodid,
                      $req->userid,
                      $req->rto_id,
                      $req->transaction_id,
                      $req->vehicleno,
                      $req->cityid,
                      $req->amount,
                      $req->payment_status,
                  	  $req->pincode,
                      $req->make,
                      $req->model
                  ));
			
			if(count($query) > 0)            	
            	return $this::send_success_response('Order Placed successfully' ,"Success",$query);
            else
        		return $this::send_failure_response("Invalid state ","failure",$query);
        }
        catch(Exception $e){
            return $this::send_failure_response($e->getMessage(),"failure",$e->getMessage());
        }
	}
}
